...
var imagenesPath = Path.Combine(wwwrootPath, "imagenes");
...
builder.Environment.WebRootPath = wwwrootPath; // Forzar el path
...
builder.Services.AddDbContext<EventosDbContext>(options =>
{
    if (useInMemory)
    {
        Console.WriteLine("[DEBUG] Configurando InMemory Database");
...
builder.Services.AddScoped<IGestorArchivos, GestorArchivosLocal>();

// Configurar MassTransit con RabbitMQ
builder.Services.AddMassTransit(x =>
{
    x.AddConsumer<Eventos.Aplicacion.Consumers.EntradaCreadaConsumer>();
    ...
    x.UsingRabbitMq((context, cfg) =>
    {
        ...
        cfg.ReceiveEndpoint("eventos-entrada-creada", e =>
        {
            e.ConfigureConsumer<Eventos.Aplicacion.Consumers.EntradaCreadaConsumer>(context);
        });
    });
});
...
// El builder ya se configuró al inicio
