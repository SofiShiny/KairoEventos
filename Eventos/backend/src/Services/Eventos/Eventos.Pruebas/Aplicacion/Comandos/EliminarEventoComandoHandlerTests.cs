...
public class EliminarEventoComandoHandlerTests
{
 ...
 {
 _repoMock.Setup(r => r.ObtenerPorIdAsync(_evento.Id, It.IsAny<bool>(), It.IsAny<CancellationToken>())).ReturnsAsync(_evento);
 ...
 {
 _repoMock.Setup(r => r.ObtenerPorIdAsync(It.IsAny<Guid>(), It.IsAny<bool>(), It.IsAny<CancellationToken>())).ReturnsAsync((Evento?)null);
 ...
 _repoMock.Verify(r=>r.ObtenerPorIdAsync(It.IsAny<Guid>(), It.IsAny<bool>(), It.IsAny<CancellationToken>()), Times.Once);
 }
}
